...
export default function HomePage() {
  const { product, setProduct, createProduct, deleteProduct, products } = productFunc();
  ...
  return (
    <div className='flex flex-col min-h-screen justify-center items-center xl:flex-row space-y-6 xl:space-y-0 space-x-0 xl:space-x-4 p-4'>
      <div className="flex flex-col space-y-4">
        ...
        {/* Form */}
        <FormWidget product={product} setProduct={setProduct} createProduct={createProduct} />
      </div>

      {/* Products */}
      <ProductWidget products={products} deleteProduct={deleteProduct} />
    </div>
  );
}
